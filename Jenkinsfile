   ...
   tools {
    ...
    stages {
        ...
        stage('Run SonarQube Analysis') {
            steps {
                script {
                        sh '/usr/local/sonar/bin/sonar-scanner -X -Dsonar.organization=ntttrang -Dsonar.projectKey=ntttrang_hello-webapp-golang -Dsonar.sources=. -Dsonar.host.url=https://sonarcloud.io'
                }
            }
        }
